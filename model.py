...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions):
        captions = captions[:,:-1]     #removing <end> , so that shape of the output matches the input dimention after concat
    ...
    def sample(self, inputs, states=None, max_len=20):
        ...
        for i in range(max_len):
            lstm_out, hidden = self.lstm(inputs,hidden)         # 1*1*HIDDEN_SIZE DIMENSION
            output_vocab = self.linear(lstm_out)                # 1*1*VOCAB_SIZE  DIMENSION
            output_vocab = output_vocab.squeeze(1)              # 1*VOCAB_SIZE    DIMENSION
            output_word = output_vocab.argmax(1)                # 1 DIMENSION
            output.append(output_word.item())
            
            inputs = self.embedding(output_word.unsqueeze(0))   # 1*1*EMBED_SIZE DIMENSION
